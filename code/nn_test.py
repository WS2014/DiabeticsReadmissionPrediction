...
#Writing outputs to NN input
def write_outs(y_scores, y_preds , y_test, s):
    y_score1, y_score2, y_score3 = y_scores
    y_pred1, y_pred2, y_pred3 = y_preds
    p_out = open('../results/NN_inputs/preds_'+s+'.csv','w')
    s_out = open('../results/NN_inputs/scores_'+s+'.csv','w')
    #
    #mapping = ['>30', 'NO', '<30']
    need_y_score1 = y_score1[:,2]
    need_y_score2 = y_score2[:,2]
    need_y_score3 = y_score3[:,2]
    z_score = np.vstack((need_y_score1, need_y_score2, need_y_score3))
    x_score_out = z_score.T
    need_y_pred1 = y_pred1[:,2]
    need_y_pred2 = y_pred2[:,2]
    need_y_pred3 = y_pred3[:,2]
    z_pred = np.vstack((need_y_pred1, need_y_pred2, need_y_pred3))
    x_pred_out = z_pred.T
    #
    #y_test
    y_out = y_test[:,2]
    #
    test_size = y_out.size
    f_line = "RF,AB,NB,y\n"
    s_out.write(f_line)
    p_out.write(f_line)
    for i in range(test_size):
        ...
        pv = x_pred_out[i]
        y_val = y_out[i]
        s_line = str(sv[0]) + "," + str(sv[1]) + "," + str(sv[2]) + "," + str(y_val) + "\n"
        p_line = str(pv[0]) + "," + str(pv[1]) + "," + str(pv[2]) + "," + str(y_val) + "\n"
        s_out.write(s_line)
        p_out.write(p_line)
...
out = open('../results/All_plot30_NN_out_train_validate.txt','w')
pp = PdfPages('../results/All_plot30_NN_out_train_validate.pdf')
...
#train-test split
x_train_ori, x_test_ori, y_train_ori, y_test_ori = train_test_split(x_all, y_all, test_size=.25,random_state=24)
x_train, x_test, y_train, y_test = train_test_split(x_train_ori, y_train_ori, test_size=.25,random_state=24)
...
clf4 = OneVsRestClassifier(AdaBoostClassifier())
...
y_pred1_ori = clf1.predict( x_test_ori )
y_pred2_ori = clf2.predict( x_test_ori )
y_pred3_ori = clf3.predict( x_test_ori )
y_score1_ori = clf1.predict_proba(x_test_ori)
y_score2_ori = clf2.predict_proba(x_test_ori)
y_score3_ori = clf3.predict_proba(x_test_ori)
###########
write_outs((y_score1, y_score2, y_score3), (pred1, pred2, pred3), y_test, 'train')
write_outs((y_score1_ori, y_score2_ori, y_score3_ori), (y_pred1_ori, y_pred2_ori, y_pred3_ori), y_test_ori, 'test')
#################################################################################
