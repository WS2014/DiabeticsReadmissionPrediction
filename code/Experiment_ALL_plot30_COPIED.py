...
train = pd.read_csv("../data/intuition/combined/complete_cleared_all_diag_compress_train_select_rand.csv", dtype={'admission_type_id':np.object, 'discharge_disposition_id':np.object, 'admission_source_id':np.object, 'diag_1':np.object, 'diag_2':np.object, 'diag_3':np.object})
test = pd.read_csv("../data/intuition/combined/complete_cleared_all_diag_compress_test_select.csv", dtype={'admission_type_id':np.object, 'discharge_disposition_id':np.object, 'admission_source_id':np.object, 'diag_1':np.object, 'diag_2':np.object, 'diag_3':np.object})

out = open('../results/All_plot30_COPIED_NODT.txt','w')
pp = PdfPages('../results/All_plot30_COPIED_NODT.pdf')
...
for i in range(y_train.size):
    y_train[i] = inv_mapping[str(y_train[i])]

for i in range(y_test.size):
    y_test[i] = inv_mapping[str(y_test[i])]
