...
def write_outs(y_scores, y_preds , y_test):
    y_score1, y_score2, y_score3 = y_scores
    y_pred1, y_pred2, y_pred3 = y_preds
    p_out = open('../results/NN_inputs/preds_train.csv','w')
    s_out = open('../results/NN_inputs/scores_train.csv','w')
    #
    #mapping = ['>30', 'NO', '<30']
...
all = pd.read_csv("../data/intuition/complete_cleared_all_diag_compress_train_select.csv", dtype={'admission_type_id':np.object, 'discharge_disposition_id':np.object, 'admission_source_id':np.object})

out = open('../results/All_plot30_NN_out_TRAIN.txt','w')
pp = PdfPages('../results/All_plot30_NN_out_TRAIN.pdf')
...
write_outs((y_score1, y_score2, y_score3), (pred1, pred2, pred3), y_test)
